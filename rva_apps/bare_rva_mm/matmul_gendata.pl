...
sub printArray
{
  ...
  print "\nstatic data_t ".$arrayName."[ARRAY_SIZE] = \n";
  print "{\n";
...
sub main()
{
  ...
  for ( my $i = 0; $i < $opts{"size"}; $i++ ) {
    for ( my $j = 0; $j < $opts{"size"}; $j++ ) {
      $mat_values1->[$i][$j] = int(rand(2));
      $mat_values2->[$i][$j] = int(rand(3));
    }
  }
  ...
  printArrayZero( "C", $opts{"size"}*$opts{"size"});
  printArrayZero( "buff", $opts{"size"});
